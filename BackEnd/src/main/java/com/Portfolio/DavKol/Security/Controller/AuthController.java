...
public class AuthController {
    ...
    public ResponseEntity<?> nuevoUsuario(@Valid @RequestBody NuevoUsuario nuevoUsuario,
        ...
        if(bindingResult.hasErrors()){
            return new ResponseEntity<>(new Mensaje("Usuario o e-mail invalido"), HttpStatus.BAD_REQUEST);
        }
        if(usuarioService.existsByUsuario(nuevoUsuario.getNombreUsuario())){
            return new ResponseEntity<>(new Mensaje("El Usuario ya está en uso"), HttpStatus.BAD_REQUEST);
        }
        if(usuarioService.existsByEmail(nuevoUsuario.getEmail())){
            return new ResponseEntity<>(new Mensaje("El e-mail ya está en uso"), HttpStatus.BAD_REQUEST);
        }
